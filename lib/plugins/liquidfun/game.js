...
ig.Box2DGame = ig.Game.extend({
	...
	loadLevel: function( data ) {
		...
		for( var i = 0; i < data.layer.length; i++ ) {
			...
			if( ld.name == 'collision' ) {
				...
				ig.particleSystem = this.createParticleSystem( ig.world, 3 );
				break;
			}
		}
    ...
    createParticleSystem: function(world, radius) {
        ...
        return world.CreateParticleSystem(systemDef);
    },
	...
	createWorldFromMap: function( origData, width, height, tilesize ) {	
		...
		var gravity = new b2Vec2( 0, this.gravity * Box2D.SCALE );
