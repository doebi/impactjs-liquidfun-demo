...
EntityPlayer = ig.Box2DEntity.extend({
    ...
    shoot: function(def) {
            ...
            shape.radius = 10 * Box2D.SCALE;
            def.shape = shape;
            def.linearVelocity = (this.flip ? new b2Vec2(-5, 0) : new b2Vec2(5, 0) );
	...
	update: function() {
		...
		if( ig.input.pressed('red') ) {
            var tpdg = this.pdg;
            tpdg.flags = b2_colorMixingParticle | b2_tensileParticle | b2_viscousParticle;
