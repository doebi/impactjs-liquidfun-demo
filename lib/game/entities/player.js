...
BaseTool = ig.Class.extend({
    ...
    spawnParticles: function(def) {
            ...
            ig.particleSystem.CreateParticleGroup(def);
    },
...
WaterNozzle = BaseTool.extend({
    ...
    primary_state: function() {
        this.pdg.shape.radius = ig.particleSystem.radius;
...
BallThrower = BaseTool.extend({
    name: "trow ball",
    init: function(bearer) {
        this.bearer = bearer;
    },
    primary_pressed: function() {
        ig.game.spawnEntity( EntityBall, this.bearer.pos, 30);
    }
});
